# Olá, eu sou o Pedro Mariano 👋

**Co-founder @ MetricHub | Estudante de Engenharia de Computação @ UTFPR**

Sou um desenvolvedor focado em construir soluções robustas que resolvem problemas reais de mercado. Atualmente, divido meu tempo entre a graduação na UTFPR e a liderança técnica da **MetricHub**, uma startup incubada focada em telemetria industrial.

---

### 🚀 Destaques e Conquistas
* [cite_start]**3º Lugar no Ideathon Inova 2025** com a MetricHub[cite: 13].
* [cite_start]**2º Lugar no Ideathon UTFPR-PB 25** com o projeto Terris[cite: 14].
* **MetricHub:** Solução de aquisição de dados (DAQ) validada pela ATLAS Eletrodomésticos, capaz de triplicar a produtividade laboratorial.
* **Terris:** Plataforma de monitoramento agrícola georreferenciado utilizando Mapbox e testes automatizados.

### 🛠️ Tecnologias e Ferramentas
* **Linguagens:** Python (Flask), TypeScript, C, Java e SQL.
* **Frontend:** React, Next.js, Tailwind CSS e Vite.
* **Data & Infra:** PostgreSQL, Edge Computing e Integração com Hardwares DAQ.
* **QA & Testes:** Vitest, Playwright e ESLint.

### 📈 Projetos em Destaque
- **[MetricHub](https://github.com/PedroI1S/MetricHub):** Backend em Python/Flask para telemetria industrial em tempo real.
- **[Terris25](https://github.com/PedroI1S/Terris25):** Agtech com mapas interativos e cobertura de testes E2E.

---

### 📫 Como me encontrar
* [cite_start]**LinkedIn:** [linkedin.com/in/pedro-mariano-b6915a300](https://www.linkedin.com/in/pedro-mariano-b6915a300) [cite: 3, 4]
* [cite_start]**Email:** pedromariano1119@outlook.com [cite: 2]
* [cite_start]**Localização:** Pato Branco, PR - Brasil [cite: 16]

---

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=PedroI1S&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=PedroI1S&layout=compact&langs_count=7&theme=tokyonight"/>
</div>
